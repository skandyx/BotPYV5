...
// --- BACKTESTING ---
const performStrategyBacktest = (klines) => {
    const bbLen = 20;
    const slPct = 3.0;
    ...
    const bbInput = { period: bbLen, values: closes, stdDev: 2 };
    ...
    try {
        for (const symbol of symbols) {
            for (const tf of timeframes) {
                try {
                    const rawKlines = await scanner.fetchKlinesFromBinance(symbol, tf, 0, 200);
                     const klines = rawKlines.map(k => ({
                        open: parseFloat(k[1]), high: parseFloat(k[2]),
                        low: parseFloat(k[3]), close: parseFloat(k[4]),
                    }));
                    const result = performStrategyBacktest(klines);
                    allResults.push({ symbol, timeframe: tf, ...result });
                } catch (e) {
                     allResults.push({ symbol, timeframe: tf, netProfitPct: 0, totalTrades: 0, winRate: 0, profitFactor: 0, maxDrawdownPct: 0, error: e.message });
                }
            }
        }
        res.json(allResults);
    } catch (error) {
        log('ERROR', `Backtest failed: ${error.message}`);
        res.status(500).json({ message: 'An internal error occurred during backtesting.' });
    }
});
